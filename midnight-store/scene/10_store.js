...
(function () {
  ...
  /* 屋顶板独立成一个有名分组（roofplate），便于俯视第三人称时整块隐藏 ——
     见 00_core.js 的 SB.ROOF_NODES。 */
  const gRoofPlate = SB.g('roofplate', gStore);
  box(10.7, 0.16, 6.2, T(0x6f6b63), -1.0, S.top + 0.08, -4.05, { parent: gRoofPlate });                // 屋顶板（略出檐）
  ...
  [0, 1].forEach(function (i) {
    ...
    /* 可玩版修正：门板与门框改放在分组原点，实际偏移交给分组位置承载。
       原版把 cx 同时写进「分组位置」和「子物体位置」，两者相加使门整体右移
       约 2.7m —— 世界包围盒落在 x≈5.0~7.0，也就是飘在小巷里，门洞是空的。 */
    const panel = new THREE.Mesh(new THREE.BoxGeometry(w, 2.72, 0.06), doorMat);
    panel.position.set(0, 0.16 + 2.72 / 2, -1.18);
    panel.renderOrder = 6; pg.add(panel);
    SB.outline(panel, 0.02);
    // 门框
    const f1 = new THREE.Mesh(new THREE.BoxGeometry(0.06, 2.72, 0.08), doorFrame); f1.position.set(i ? -w / 2 : w / 2, 0.16 + 2.72 / 2, -1.17); pg.add(f1);
    const f2 = new THREE.Mesh(new THREE.BoxGeometry(0.06, 2.72, 0.08), doorFrame); f2.position.set(i ? w / 2 : -w / 2, 0.16 + 2.72 / 2, -1.17); pg.add(f2);
    const f3 = new THREE.Mesh(new THREE.BoxGeometry(w, 0.06, 0.08), doorFrame); f3.position.set(0, 0.19, -1.17); pg.add(f3);
    const f4 = new THREE.Mesh(new THREE.BoxGeometry(w, 0.06, 0.08), doorFrame); f4.position.set(0, 2.85, -1.17); pg.add(f4);
    const f5 = new THREE.Mesh(new THREE.BoxGeometry(w, 0.05, 0.08), doorFrame); f5.position.set(0, 1.35, -1.17); pg.add(f5);
