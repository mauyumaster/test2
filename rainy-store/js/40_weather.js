...
(function () {
  ...
  for (let i = 0; i < RAIN_N; i++) {
    drops.push({ x: SB.rnd(-9.5, 9.5), y: SB.rnd(-1, 14), z: SB.rnd(-9.5, 9.5), sp: SB.rnd(13, 24), len: SB.rnd(0.5, 1.05) });
  }
  ...
  for (let i = 0; i < RAIN2; i++) drops2.push({ x: SB.rnd(-6, 8), y: SB.rnd(-1, 8), z: SB.rnd(-3, 9), sp: SB.rnd(18, 30), len: SB.rnd(0.9, 1.6) });
  ...
  rain2.frustumCulled = false; G.add(rain2);

  function stepRain(arr, data, geo, pos, dt) {
    for (let i = 0; i < data.length; i++) {
      const d = data[i];
      d.y += DIR.y * d.sp * dt; d.x += DIR.x * d.sp * dt; d.z += DIR.z * d.sp * dt;
      if (d.y < -1.2) { d.x = SB.rnd(-9.5, 9.5); d.z = SB.rnd(-9.5, 9.5); d.y = SB.rnd(11, 15); }
      const g = i * 6;
  ...
  SB.anim.push(function (t, dt) {
    /* 降雨 */
    stepRain(rain, drops, rainGeo, rpos, dt);
    stepRain(rain2, drops2, rainGeo2, rpos2, dt);
    ...
    for (let i = 0; i < ripples.length; i++) {
      ...
      r.m.material.opacity = 0.5 * (1 - u) * (1 - u);
    }
